...
public class PageGridView extends RecyclerView {
    ...
    public PageGridView(Context context, AttributeSet attrs, int defStyle) {
        ...
        //添加分割线
        if (diver != null)
            addItemDecoration(new DividerGridItemDecoration(getContext(), diver));
    }
    ...
    private int getPositionByXY(int x, int y) {
        ...
        if (mRows > 0) {
            int offset = getChildLayoutPosition(getLayoutManager().getChildAt(0));
            position += offset;
        }
        return position;
    }
    ...
    public void not(Adapter adapter) {
        needReorder = mRows != 0;
        pageIndicaotrNeedInit = true;
        currentPage = 0;
        scrollX = 0;
